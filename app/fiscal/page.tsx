...
import { Receipt, TrendingUp, TrendingDown, ChevronDown, ChevronUp, Info, Download } from 'lucide-react'

interface FiscalData {
  year: number
  plusValues: {
    total: number; cto: number; pea: number; exoneres: number
    lines: Array<{ date: string; symbol: string; quantity: number; sellPrice: number; avgBuyPrice: number; plusValue: number; accountType: string; exonere: boolean }>
  }
  dividends: {
    total: number
    lines: Array<{ date: string; symbol: string | null; amount: number; currency: string }>
  }
  tax: {
    taxableBase: number; pfuAmount: number; pfuRate: number
    peaExonereAmount: number; irAmount: number; socialAmount: number
  }
}
...
export default function FiscalPage() {
  ...
  return (
    <div className="flex flex-col min-h-screen">
      <Header title="Rapport fiscal" subtitle="Plus-values, dividendes et estimations d'impôt" />
          ...
          <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <Card className="border-accent/30 bg-accent/5">
                <CardContent className="pt-5">
                  <p className="text-text-muted text-xs mb-1">Impôt estimé (PFU 30%)</p>
              ...
              <Card>
                <CardContent className="pt-5">
                  <p className="text-text-muted text-xs mb-1">Plus-values réalisées</p>
                  ...
                  <p className="text-text-muted text-xs mt-1">CTO : {formatCurrency(data.plusValues.cto)} · PEA : {formatCurrency(data.plusValues.pea)}</p>
                </CardContent>
              </Card>
              <Card>
                <CardContent className="pt-5">
                  <p className="text-text-muted text-xs mb-1">Dividendes perçus</p>
                  <p className="text-2xl font-bold font-mono text-emerald-400">{formatCurrency(data.dividends.total)}</p>
                  <p className="text-text-muted text-xs mt-1">PEA exonérés (PS) : {formatCurrency(data.plusValues.exoneres)}</p>
                </CardContent>
              </Card>
            </div>
            ...
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center gap-2 text-base">
                  <Receipt className="w-4 h-4 text-accent" /> Décomposition du PFU
                </CardTitle>
              </CardHeader>
              <CardContent className="space-y-3">
                <div className="bg-surface-2 rounded-xl p-4 space-y-2.5">
                  {[
                    ...
                    { label: 'Prélèvements sociaux (17,2%)', val: data.tax.socialAmount },
                    { label: 'PS PEA exonéré d\'IR',    val: data.tax.peaExonereAmount },
                  ].map(r => (
                  ...
                  <div className="border-t border-border pt-2 flex justify-between text-sm font-semibold">
                    <span className="text-text-primary">Total estimé</span>
                ...
                <div className="flex items-start gap-2 text-xs text-text-muted bg-surface-2 rounded-xl px-4 py-3">
                  ...
                  Estimation indicative basée sur le PFU 30%. Consultez un conseiller fiscal pour votre déclaration officielle.
                </div>
              </CardContent>
            </Card>
              ...
              <Card>
                <CardHeader>
                  <button onClick={() => setShowLines(v => !v)}
                    ...
                    <CardTitle className="flex items-center gap-2 text-base">
                      <TrendingUp className="w-4 h-4 text-emerald-400" /> Détail des cessions ({data.plusValues.lines.length})
                    </CardTitle>
                    {showLines ? <ChevronUp className="w-4 h-4 text-text-muted" /> : <ChevronDown className="w-4 h-4 text-text-muted" />}
                  </button>
                </CardHeader>
                {showLines && (
                        ...
                        <tbody>
                          {data.plusValues.lines.map((l, i) => (
                            <tr key={i} className="border-b border-border/50 hover:bg-surface-2/50">
                              <td className="px-5 py-3 text-text-muted text-xs">{new Date(l.date).toLocaleDateString('fr-FR')}</td>
                              ...
                              <td className="px-4 py-3">
                                <span className={cn('text-xs px-2 py-0.5 rounded-lg font-medium',
                                  ...
                                  {l.exonere ? 'PEA ✓' : l.accountType}
                                </span>
                              </td>
                              <td className="px-4 py-3 text-right font-mono text-text-secondary">{l.quantity.toLocaleString('fr-FR')}</td>
                              <td className="px-4 py-3 text-right font-mono text-text-secondary">{formatCurrency(l.avgBuyPrice)}</td>
                              <td className="px-4 py-3 text-right font-mono text-text-secondary">{formatCurrency(l.sellPrice)}</td>
                              <td className={cn('px-5 py-3 text-right font-mono font-semibold', l.plusValue >= 0 ? 'text-emerald-400' : 'text-red-400')}>
            ...
            {data.dividends.lines.length > 0 && (
              <Card>
                <CardHeader>
                  <CardTitle className="flex items-center gap-2 text-base">
                    <Receipt className="w-4 h-4 text-accent" /> Dividendes perçus
                  </CardTitle>
                </CardHeader>
                <CardContent className="p-0">
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {data.dividends.lines.map((d, i) => (
                        <tr key={i} className="border-b border-border/50 hover:bg-surface-2/50">
                          ...
                          <td className="px-5 py-3 text-right font-mono font-semibold text-accent">+{formatCurrency(d.amount, d.currency)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
            ...
            {data.plusValues.lines.length === 0 && data.dividends.lines.length === 0 && (
              <div className="flex flex-col items-center justify-center py-16 text-center">
                <p className="text-text-primary font-semibold mb-1">Aucune opération fiscale en {year}</p>
                <p className="text-text-muted text-sm">Enregistrez vos ventes et dividendes dans les transactions.</p>
              </div>
            )}
          </>
        )}
